...
def get_artists():
    try:
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT artists.name, artists.age, countries.name FROM artists JOIN countries ON artists.country = countries.id")
        results = cur.fetchall()
        cur.close()
...
def display_users_table():
    try:
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT users.id, users.username, users.password, museums.name AS museum_name, museums.location FROM users LEFT JOIN museums ON users.museum_id = museums.id")
        users = cur.fetchall()
        cur.close()
...
def add_user():
    try:
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO users (username, password, museum_id) VALUES (%s, %s, %s)",
                    (username, password, museum_id))
        mysql.connection.commit()
        cur.close()
